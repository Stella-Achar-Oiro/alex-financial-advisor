...
def create_agent(job_id: str, portfolio_summary: Dict[str, Any], db):
    ...
    # Create explicit task — Nova Pro requires directive instructions to call all tools
    task = f"""You are orchestrating financial analysis for job {job_id}.
Portfolio: {portfolio_summary['num_positions']} positions across {portfolio_summary['num_accounts']} accounts.
Retirement: {portfolio_summary['years_until_retirement']} years away.

You MUST call ALL THREE tools in this exact order:
1. invoke_reporter — generates the portfolio analysis report
2. invoke_charter — generates interactive chart data
3. invoke_retirement — generates retirement projections

Call invoke_reporter now, then invoke_charter, then invoke_retirement. Do not skip any."""
